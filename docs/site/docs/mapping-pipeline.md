...
2. **Value mapping**: For attributes with enumerated values, the
   qualifier's `request_value_map` translates Rust values to MQSC
   values (e.g. `"yes"` → `"YES"`).
...
3. **Key-value mapping**: Some attributes require both key and value to
   ...
   where a single Rust attribute expands to a different MQSC key+value
...
2. **Value mapping**: Enumerated MQSC values are translated to
   Rust-friendly values via the `response_value_map` (e.g. `"YES"` →
   `"yes"`).
...
1. Looking up the command key (e.g. `"DISPLAY QUEUE"`) in
   the command metadata for an explicit qualifier.
...
shapes return errors immediately, so problems are caught before any commands
are sent.
